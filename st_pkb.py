        ...
        if parsed_name.hasName():
            ...
            if parsed_name.hasAuthorship():
                ...
                if len(df) == 0:
                    ...
                    df = wr.neptune.execute_gremlin(client, query)
                    if len(df) == 0:
                        print("The extracted taxon seems to have typo, we'll try to suggest taxon based on authorship, genus and the first three characters of the specificEpithet")
                        taxon_genus = parsed_name.genus
                        try:
                            # Try to assign the first three characters of specificEpithet to taxon_s
                            if len(parsed_name.specificEpithet) >= 2:
                                taxon_s = parsed_name.specificEpithet[0:2]
                                taxon_g_s = taxon_genus + " " + taxon_s
                            else:
                                taxon_s = None
                                taxon_g_s = taxon_genus
                        except AttributeError:
                        # Handle the case where specificEpithet attribute doesn't exist
                            taxon_s = None  # Assign a default value or handle it in another way
                        query = f"g.V().has('taxon', 'authorship', '{taxon_authorship}').has('taxon', 'name', TextP.containing('{taxon_g_s}')).valueMap(true)"
                        df = wr.neptune.execute_gremlin(client, query)
            else:
                query = f"g.V().has('taxon','name','{taxon_name}').valueMap(true)"
                df = wr.neptune.execute_gremlin(client, query)
                if len(df) == 0:
                    print("The extracted taxon seems to have typo, we'll try to suggest taxon based on genus and the first three characters of the specificEpithet")
                    taxon_genus = parsed_name.genus
                    try:
                        # Try to assign the first three characters of specificEpithet to taxon_s
                        if len(parsed_name.specificEpithet) >= 2:
                        ...
                        taxon_s = None  # Assign a default value or handle it in another way
                    print(taxon_g_s)
                    query = f"g.V().has('taxon', 'name', TextP.containing('{taxon_g_s}')).valueMap(true)"
                    df = wr.neptune.execute_gremlin(client, query)
            return df
...
def print_collector_recommended(df):
    for index, row in df.iterrows():
        ...
        # Check and print Fullname
        full_name = "; ".join(filter(pd.notna, [
            row.get('fullname_w', None), 
            row.get('fullname1_h', None),
            row.get('fullname2_h', None),
            row.get('fullname_b', None)
        ]))
        if author_abbrv:
            st.text("Fullname: " + full_name)
...
if uploaded_file is not None:
    ...
    # namelist = [name for collectorname in temp for name in split_and_clean_names(collectorname)]
    # namelist = split_and_clean_names(result_df['collectorname'].tolist()[0])
    # st.text(namelist)
    # st.text(namelist[0])
    # st.text(split_and_clean_names(namelist[0]))
    namelist = split_and_clean_names(namelist[0])
    # st.text(namelist)
    ...
    st.text("Trying to find possible matched GBIF records with country, taxon and collector information.")
    if not is_valid(namelist[0]):
        ...
        if len(namelist) >= 1:
            ...
            for name in namelist:
                temp_df = search_specimen_in_pkb(countrycode,taxon_df.iloc[0]['name'],name)
                result_df = pd.concat([result_df, temp_df], ignore_index=True)
            result_df = result_df.drop_duplicates(subset='gbifid')
            in_gbif_df = result_df
    ...
    if len(namelist) >= 1:
        if not is_valid(namelist[0]):
            ...
            if len(namelist) >= 2:
                ...
                # st.text(nameinfo)
                cleaned_name = nameinfo.replace("'", "")
                # st.text(cleaned_name)
                recordby_df = get_recordedby_with_collector_info(cleaned_name)
                # st.text("nameinfo done")
            else:
                result_df = pd.DataFrame()
                for name in namelist:
                    # st.text(name)
                    temp_df = get_recordedby_with_collector_info(name)
                    result_df = pd.concat([result_df, temp_df], ignore_index=True)
                result_df = result_df.drop_duplicates(subset='collectorindex')
                recordby_df = result_df
            ...
            st.dataframe(recommend_collector_df)
